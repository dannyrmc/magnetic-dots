...
const InteractiveDots = forwardRef<InteractiveDotsRef, InteractiveDotsProps>(
  ({ densityLevel = 12, dotSizeScaler = 1.5, animationSpeed = 0.008 }, ref) => {
    ...
    // Enhanced haptic feedback function with multiple fallbacks
    const triggerHapticFeedback = () => {
      const now = Date.now()
      // Throttle vibrations to avoid overwhelming the user (max once per 150ms)
      if (now - lastVibrationRef.current < 150) return

      try {
        // Method 1: iOS Haptic Feedback (iOS 10+)
        if ("DeviceMotionEvent" in window && "requestPermission" in (DeviceMotionEvent as any)) {
          // This is iOS - try the Haptic Feedback API
        ...
        // Method 2: Web Vibration API (Android)
        if ("vibrate" in navigator) {
          navigator.vibrate(20) // Slightly longer for better feel
          lastVibrationRef.current = now
          return
        }

        // Method 3: Audio feedback as fallback (works on all platforms)
        // Create a very short, low-volume audio click
        ...
        // Silently fail - haptic feedback is nice-to-have, not essential
    ...
    useEffect(() => {
      ...
      const updateCanvasSize = () => {
        ...
        const setCanvasSize = (cnv: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
        ...
        dotsRef.current = generateDots(rect.width, rect.height, initialAnimationRef.current, dotDensity, dotSizeScaler)
      }
    ...
    const handleInteraction = (clientX: number, clientY: number) => {
      ...
      const hasMoved = clientX !== prevMouseRef.current.x || clientY !== prevMouseRef.current.y
    ...
    return (
      <div ref={containerRef} className="relative h-full w-full">
        ...
        <MouseTracker onMouseMove={handleInteraction} onMouseLeave={handleMouseLeave} />
      </div>
    )
  },
)
